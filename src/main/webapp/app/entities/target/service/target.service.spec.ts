...
import { sampleWithRequiredData, sampleWithNewData, sampleWithPartialData, sampleWithFullData } from '../target.test-samples';

import { TargetService } from './target.service';

const requireRestSample: ITarget = {
...
describe('Target Service', () => {
  ...
  describe('Service methods', () => {
    ...
    it('should create a Target', () => {
      ...
      const target = { ...sampleWithNewData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.create(target).subscribe(resp => (expectedResult = resp.body));
    ...
    it('should update a Target', () => {
      const target = { ...sampleWithRequiredData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.update(target).subscribe(resp => (expectedResult = resp.body));
    ...
    describe('addTargetToCollectionIfMissing', () => {
      it('should add a Target to an empty array', () => {
        const target: ITarget = sampleWithRequiredData;
      ...
      it("should add a Target to an array that doesn't contain it", () => {
        const target: ITarget = sampleWithRequiredData;
        const targetCollection: ITarget[] = [sampleWithPartialData];
        expectedResult = service.addTargetToCollectionIfMissing(targetCollection, target);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(target);
      });
      ...
      it('should add only unique Target to an array', () => {
        const targetArray: ITarget[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const targetCollection: ITarget[] = [sampleWithRequiredData];
      ...
      it('should accept varargs', () => {
        const target: ITarget = sampleWithRequiredData;
        const target2: ITarget = sampleWithPartialData;
    ...
    describe('compareTarget', () => {
      it('Should return true if both entities are null', () => {
        ...
        const compareResult = service.compareTarget(entity1, entity2);
      ...
      it('Should return false if one entity is null', () => {
        ...
        const compareResult1 = service.compareTarget(entity1, entity2);
        const compareResult2 = service.compareTarget(entity2, entity1);
