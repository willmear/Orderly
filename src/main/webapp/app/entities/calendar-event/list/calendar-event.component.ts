...
export class CalendarEventComponent implements OnInit {
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.calendarEvents = this.refineData(dataFromBody);
  }

  protected refineData(data: ICalendarEvent[]): ICalendarEvent[] {
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    const queryObject = {
      eagerload: true,
      allEvents: true,    //request that all events from all users are returned - backend will only allow this if current user is admin
